//Adaptation of Eliptical Propeller Generator by user_3514236347 on makerworld https://makerworld.com/en/@user_3514236347 Originaly Created By Amatulic On Thingiverse https://www.thingiverse.com/amatulic/designs

Number_of_Blades = 3;

Propeller_Diameter = 127;//0.1

Hub_Diameter = 12.5;//0.1

Hole_Diameter = 5;//0.1

//Blade pitch - typically between 75% to 133% of blade length for aircraft. Lower pitch has better climb performance, higher pitch has better cruise performance. Defaults to same value as propeller arc radius if negative.
Blade_Pitch = -1;

//Maximum possible chord length for zero attack angle blade, as a fraction of blade length.
Max_Chord_Fraction = 0.15;//0.01

//Height of the root of the propeller.
Root_Length = 4.5;//0.1
...
Trailing_Edge_Thickness = 0.2;//0.01

//Millimeters to thicken the root airfoil, transitioning to normal thickness this many millimeters from the hub.
Fairing_Size = 4;//0.1
//Airfoils Must Be from the NACA 4 Digit series of Airfoils
Root_NACA_Airfoil = 9425;

Mid_NACA_Airfoil = 4412;

Tip_NACA_Airfoil = 3410;

//Fraction of blade length where root airfoil completes its transition to mid airfoil. Set to zero to disable root transition
Transition_Point = 0.3;//0.01

//Set to 1 for spinner
Parabolic_Spinner_Yes = 0;//1
...
//Set to 1 for spinner
Ogive_Spinner_Yes = 0;//1

Ogive_Spinner_Diameter = 20;

Ogive_Spinner_Length = 20;

Nose_Radius = 0.24;

Hoop = 0;//1

Hoop_Thickness = 1.2;//0.1
...
//function to generate the propeller
    color("green")
    translate([0,0,Prop_Z_Offset])
        propeller(blades=Number_of_Blades, propdia=     Propeller_Diameter, hubdia=Hub_Diameter, bladepitch=Blade_Pitch, maxchordfrac=Max_Chord_Fraction, hublen=Root_Length, elenfrac=Elen_Fraction, dir=Prop_Direction, centerline=Centerline, angle_sweep=Sweep_Angle, te_thickness=Trailing_Edge_Thickness, fairing=Fairing_Size, naca_root=Root_NACA_Airfoil, naca_mid=Mid_NACA_Airfoil, naca_tip=Tip_NACA_Airfoil, root_transition=Transition_Point, profilepoints=0, slices=0);
    //function to generate hub
    difference() {
        cylinder(Hub_Length, d=Hub_Diameter, $fn=60);
        translate([0,0,-0.5])
        cylinder(Hub_Length+1.5, d=Hole_Diameter, $fn=60);
        echo("Blade Length =",0.5*Propeller_Diameter - (Hub_Diameter/2));}

if (Parabolic_Spinner_Yes == 1){
    difference() {
        paraboloc_spinner(length=Spinner_Length, diameter=Spinner_Diameter);
        translate([0,0,-0.5])
        cylinder(Hub_Length+1.5, d=Hole_Diameter, $fn=60);}
}
if (Ogive_Spinner_Yes == 1){
    difference() {
        ogive_spinner(length=Ogive_Spinner_Length, diameter=Ogive_Spinner_Diameter, noseradius=Nose_Radius);
        translate([0,0,-0.5])
        cylinder(Hub_Length+1.5, d=Hole_Diameter, $fn=60);}
}
if (Hoop == 1){
    translate([0,0,Hoop_Offset])
        difference() {
            cylinder(Hoop_Height, d=Propeller_Diameter+2*Hoop_Thickness, $fn=100);
        translate([0,0,-0.5])
        cylinder(Hoop_Height+1.5, d=Propeller_Diameter, $fn=100);}
    }
